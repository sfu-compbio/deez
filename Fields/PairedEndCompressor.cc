...
void PairedEndCompressor::outputRecords (const CircularArray<Record> &records, Array<uint8_t> &out, size_t out_offset, size_t k, const CircularArray<PairedEndInfo> &pairedEndInfos) 
{
	if (!records.size()) { 
		out.resize(0);
		return;
	...
	for (size_t i = 0; i < k; i++) {
		...
		if (pairedEndInfos[i].bit == PairedEndInfo::Bits::LOOK_BACK) {
			int32_t tlen = pairedEndInfos[i].tlen;
			assert(tlen > 0);
			if (tlen < (1 << 16)) {
				uint16_t sz16 = tlen;
				assert(sz16 > 0);
		...
		} else if (pairedEndInfos[i].bit >= PairedEndInfo::Bits::OK) {
